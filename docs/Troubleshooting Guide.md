...
Each HTTP Request node needs the auth header. In n8n, open the failing workflow, find the **Call LM Studio** HTTP Request node, and verify:
```
Authorization: Bearer <your LM_STUDIO_API_KEY>
```
The key is stored in `/docker/mcp/.env` as `LM_STUDIO_API_KEY`.
...
- All parse-response Code nodes fall back to `msg.reasoning_content` if `msg.content` is empty

If you see empty output again, check that the parse-response node in the failing workflow has:
```javascript
const content = (msg.content && msg.content.trim()) ? msg.content : (msg.reasoning_content || '');
```

If responses are truncated, increase `max_tokens` in the Build Request node.
...
The Bearer token in the **Write Files to Disk** HTTP node in `07-Task-Processor` must match `FILE_API_TOKEN` in `/docker/stacks/file-api/.env`. They should both be the same value from `/docker/mcp/.env`.

### Files not appearing on disk

Check the File API is running:
```bash
docker ps | grep file-api
curl -H "Authorization: Bearer <token>" http://localhost:3456/health
```

Files land at `/home/will/src/{project_id}/`. The File API runs as the `node` Docker user, so files will be owned by that user — use `sudo` or the File API's DELETE endpoint to remove them from the host.

### Path traversal blocked (unexpected 500)

The File API rejects any path that escapes the project root. Ensure the `path` values in the `[{path, content}]` output from Code Writer are relative (e.g., `src/index.ts`, not `/etc/passwd` or `../../etc/passwd`).
...
`getWorkflowStaticData('global')` persists as long as the **06-Project-Memory** workflow is not re-imported or deleted. Re-importing a workflow resets its static data. If you need to re-import, export the static data first (it's stored in the n8n SQLite DB under the workflow's `staticData` column).
...
3. Are all 8 workflows activated in n8n?
4. Is the file-api container running?
5. Does the webhook URL include the workflow ID prefix?
