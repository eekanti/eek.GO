...
3. If the IP changed: update the `url` field in each workflow's HTTP Request node

### 401 Unauthorized from LM Studio

All agents use HTTP Request nodes with `Authorization: Bearer $env.LLM_API_KEY`.
...
If responses are truncated, increase `max_tokens` in the Build Request node (Planner/Combined Reviewer) or the Prepare Message node (Code Writer).
...
- Workflows that call others: `00-Master-Orchestrator` → `01`, `06`, `07`; `07-Task-Processor` → `09`; `09-Chunk-Processor` → `02`, `08`
...
### Code Writer not reflecting previous feedback (P2)

The Code Writer uses direct HTTP (no Redis or session memory). On P2 retry, the full task context and `previous_feedback` string are re-sent in the same request. If feedback isn't being applied:

1. Open the failing execution in n8n
2. Check the **Build Retry Input** node output in `09-Chunk-Processor` — confirm `previous_feedback` is populated and non-empty
3. Check the **Call Code Writer Retry** node response — confirm the model acknowledged the feedback

### Model only outputs partial files

The Code Writer sends a `FILES TO MODIFY` constraint from `task.files` (filtered to the current chunk). If the Planner didn't include a `files` array in the task, no constraint is sent and the model may produce extra files. Re-run the planner or manually add a `files` array to the task input.

---

## Chunk Processor / Quality Gate

### Tasks stuck retrying

The review gate passes when `(job_complete === true OR quality_score >= 80) AND critical_issues.length === 0`.

After 2 passes (P1 → P2), the Chunk Processor writes files regardless. If your model never returns `job_complete: true`, change the condition in the **Review Gate** Code node in `09-Chunk-Processor`:
...
### Combined Reviewer returning unparseable output

If the Combined Reviewer returns `raw_output` (parse failure), the Chunk Processor accepts the code as-is. Check LM Studio is returning valid JSON in reviewer responses. Increase `max_tokens` in the `Build Request` node of `08-Combined-Reviewer-Agent` if responses appear truncated.

### Chunks not progressing

The Task Processor uses `getWorkflowStaticData('global')` to queue chunks. If an execution crashes mid-run, stale state may be left in static data. To clear it, open `07-Task-Processor` in n8n → Settings → clear static data (or re-import the workflow, which resets it).
...
The Bearer token in the **Write Files to Disk** HTTP node in `09-Chunk-Processor` must match `FILE_API_TOKEN` in `/docker/stacks/file-api/.env`.
...
The Research Agent connects to Context7 MCP at `http://docky:8811/mcp`. If the docky container is not running, the agent skips research gracefully (returns empty `_reference`). To verify:

```bash
docker ps | grep docky
curl http://docky:8811/mcp  # from inside the n8n container's Docker network
```

Library detection is keyword-based. If your library isn't detected automatically, it may not be in the supported list (React, HeroUI, Tailwind, Vite, Next.js, Express, Prisma, Drizzle, Fastify, Hono, Zod, tRPC, Framer Motion). Add it to the `Extract Libraries` Code node in `10-Research-Agent`.
...
4. Are all workflows activated in n8n? (00, 01, 02, 05, 06, 07, 08, 09 are required; 10 is optional)
